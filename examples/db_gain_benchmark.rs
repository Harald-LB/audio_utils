use audio_utils::decibels::db_to_gain;
...
fn main() {
    ...
    // Per-call
    let lut_pc = measure_per_call("LUT per-call", &xs, per_call_iters, db_to_gain);
    let pow_pc = measure_per_call("powf per-call", &xs, per_call_iters, powf_ref);

    // Batch
    let lut_bs = measure_batch_sweep("LUT batch-sweep", &xs, batch_sweeps, db_to_gain);
